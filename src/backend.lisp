...
(defun token-bucket-decrement (token-bucket n &optional with-punishment)
  (let* ((time (get-internal-real-time))
    ...
    (declare (type (and fixnum (integer 0)) time)
	     (type (unsigned-byte 64) limit cost max-increment increment)
...
(sb-ext:defglobal *global-token-bucket* (make-token-bucket :rate 30 :burst 180))
