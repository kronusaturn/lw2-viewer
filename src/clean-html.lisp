          ...
          (let ((wayward-li-container nil))
            (plump:traverse root (lambda (node)
                                   (cond
                                     ...
                                     ((tag-is node "blockquote")
                                      (unless (every (lambda (n) (or (and (plump:text-node-p n) (string-is-whitespace (plump:text n)))
                                                                     (and (plump:element-p n) (tag-is n "p"))))
                                                     (plump:children node))
                                        (let ((new-p (plump:make-element root "p")))
                                      ...
                                      (loop with current-node = (plump:next-sibling node)
                                            while current-node
                                            for next-node = (plump:next-sibling current-node)
                                            do (cond ((and (plump:text-node-p current-node) (string-is-whitespace (plump:text current-node)))
                                                      (plump:remove-child current-node))
                                                     ((and (plump:element-p current-node) (tag-is current-node "blockquote"))
                                                      (progn
                                                        (plump:remove-child current-node)
                                                        (setf (plump:tag-name current-node) "p")
                                                        (plump:append-child node current-node)))
                                                     (t (return)))
                                            do (setf current-node next-node)))))
