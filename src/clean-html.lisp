                                   ...
                                   (cond
                                     ((tag-is node "ul" "ol")
                                      ...
                                      (loop for child across (plump:children node)
                                            unless (or (and (plump:element-p child) (tag-is child "li"))
                                                       (and (plump:text-node-p child) (string-is-whitespace (plump:text child))))
                                            do (let ((ps (or (plump:previous-sibling child) (let ((ps (plump:make-element root "li"))) (plump:remove-child ps) (plump:insert-before child ps) ps))))
                                                 (plump:remove-child child)
                                                 (plump:append-child ps child))))
                                     ((tag-is node "li")
                                      (unless (is-child-of-tag node "ul" "ol")
                                        (unless wayward-li-container
                                          (setf wayward-li-container (plump:make-element root "ul"))
                                          (plump:remove-child wayward-li-container)
                                          (plump:insert-before node wayward-li-container))
                                       ...
                                       ((tag-is node "p" "blockquote" "div")
                                        (if (string-is-whitespace (plump:text node))
                                            (if (plump:get-elements-by-tag-name node "img")
                                                ...
                                                (plump:remove-child node))
                                            (if (loop for e = (first-non-whitespace-child node) then (first-non-whitespace-child e)
                                                      unless (and e (typep e 'plump:element)) return nil
                                                      when (search "mjx-math" (plump:attribute e "class")) return t)
