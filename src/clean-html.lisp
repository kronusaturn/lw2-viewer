...
(defvar *mathjax-process* nil)

(defmacro with-mathjax-processor (() &body body)
  `(let ((*mathjax-process* nil))
     (unwind-protect
	  (progn ,@body)
       (when *mathjax-process*
	 (uiop:close-streams *mathjax-process*)
	 (uiop:wait-process *mathjax-process*)))))

(defun start-mathjax-process ()
  (unless *mathjax-process*
    (setf *mathjax-process* (uiop:launch-program "node js-foreign-lib/mathjax.js"
						 :input :stream :output :stream
						 :element-type '(unsigned-byte 8))))
  (values (uiop:process-info-input *mathjax-process*)
	  (uiop:process-info-output *mathjax-process*)))

(defun process-mathjax (tex)
  (multiple-value-bind (outstream instream) (start-mathjax-process)
    (let ((outbuf (babel:string-to-octets tex)))
      (nibbles:write-ub32/le (length outbuf) outstream)
      (write-sequence outbuf outstream)
      (finish-output outstream))
    (let ((insize (nibbles:read-ub32/le instream)))
      (when (> insize 0)
	(let ((inbuf (make-array insize :element-type '(unsigned-byte 8))))
	  (read-sequence inbuf instream)
	  (babel:octets-to-string inbuf))))))

(defun handle-codecogs (node uri)
  (regex-case uri
	      ("^https?://[a-z.]*codecogs\.com/[a-z.]*\\?(.*)$"
	       (declare (regex-groups-min 1))
	       (let ((math-html (process-mathjax (quri:url-decode (reg 0)))))
		 (when math-html
		   (let* ((math-dom (plump:parse math-html))
			  (children (plump:children math-dom)))
...
(define-lmdb-memoized clean-html 'lw2.backend-modules:backend-lmdb-cache
  ...
  (labels ((only-child-is (node &rest args)
	...
	(let ((root (plump:parse (string-trim '(#\Space #\Newline #\Tab #\Return #\Linefeed #\Page) in-html)))
	  ...
	  (loop while (and (= 1 (length (plump:children root)))
			   (plump:element-p (plump:first-child root))
			   (tag-is (plump:first-child root) "div" "html" "body")
			   (not (spoilerp (plump:first-child root))))
		  ...
		  (cond
		    ...
		    ((tag-is node "img")
		     (let ((width (ignore-errors (parse-integer (plump:attribute node "width"))))
			   ...
			   (let ((container
			     ...
			     (setf (plump:attribute node "loading") "lazy")))))
		    ((tag-is node "figure")
		     (remove-attributes node "style" "class" "width" "height"))
		    ((and (tag-is node "p") (only-child-is node "figure"))
		     (move-children-out-of-node node))
		    ((and (tag-is node "div") (nonempty-string (plump:attribute node "data-iframe-widget-id")))
		     (let ((widget-id (plump:attribute node "data-iframe-widget-id")))
		       (plump:make-text-node node "[Embedded widget]")
		       (create-dynamic-call node 'lw2.iframe-widget:render-iframe-widget widget-id)))
		    ((tag-is node "p" "blockquote" "div" "center")
		     ...
		     (when (tag-is node "center")
		       (setf (plump:tag-name node) "p")
		       (add-element-style node "text-align" "center"))
		     (when-let ((question-id (plump:attribute node "data-elicit-id")))
		       (when-let* ((question-title (lw2.backend::get-elicit-question-title question-id))
				   (links (plump:get-elements-by-tag-name node "a"))
				   (text-node (plump:first-child (first links))))
			 (setf (plump:text text-node) question-title))
		       (let ((target node))
			 (dotimes (n 2)
			   (when (not (plump:root-p (plump:parent target)))
			     (setf target (plump:parent target)))
			   (create-dynamic-call target 'lw2.elicit-predictions::render-elicit-block question-id))))
		     (when (and (direct-class-is node "llm-content-block")
				(not (nonempty-string (plump:attribute node "data-model-name"))))
		       (setf (plump:attribute node "data-model-name") "AI-generated"))
		    ...
		    ((tag-is node "table" "tbody" "tr" "td")
		     ...
		     (when (and (tag-is node "td") (is-numeric node))
		       (add-class node "numeric")))
		    ((tag-is node "u")
	  ...
	  (let ((with-toc (>= section-count 3))
		...
		(format out-stream "<style>~%:root, body.force-light-mode {~%")
		(maphash (lambda (name rgba-list)
			   (declare (ignore rgba-list))
			   (format out-stream "  --user-color-~A: #~A;~%" name name))
			 used-colors)
		(flet ((write-inverted-colors (selector)
			 (format out-stream "~A {~%" selector)
			 (maphash (lambda (name rgba-list)
		  ...
		  (write-inverted-colors ":root")
		  (format out-stream "}~%")
		  (write-inverted-colors "body.force-dark-mode"))
