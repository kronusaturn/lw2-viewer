...
(defgeneric rectify-post* (backend post) ; TODO this should go in a more generic postprocessing method
  ...
  (:method ((backend backend-lw2-misc-workarounds) post)
    (let* ((post (if (next-method-p) (call-next-method) post))
	   (url-cons (assoc :url post))
	   (url (cdr url-cons))
	   (url (and url (nonempty-string (string-trim " " url))))
	   (url (and url (cond ((or (uiop:string-prefix-p "http" url) (uiop:string-prefix-p "/" url)) url)
			       (t (concatenate 'string "http://" url))))))
      (cond ((eq url (cdr url-cons)) post)
	    (t (acons :url url post)))))
