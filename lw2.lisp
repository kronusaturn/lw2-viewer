  ...
  (with-response-context ()
	...
	(return-from call-with-error-page t))
	  ...
	  (cond ((or (let ((revalidate-header (hunchentoot:header-in* :x-revalidate)))
		     ...
		     (ppcre:scan "GPTBot|ClaudeBot|GoogleOther|AmazonBot|facebookexternalhit|meta-externalagent|FriendlyCrawler|LinkupBot|SleepBot"
   ...
   (:get ()
	 ...
	 (let* ((is-event (cdr (assoc :is-event post)))
	   ...
	   (when (string/= post-type correct-subtype)
	     ...
	     (return)))
	 (labels ((extra-head ()
		    (when-let (canonical-source (or (if (not comment-id)
							(cdr (assoc :canonical-source post)))
						    (if (always-canonical *current-site*)
						    ...
						    (if (and (not comment-id)
							     (or (hunchentoot:get-parameter "hide-nav-bars")
								 (hunchentoot:get-parameter "comments")))
