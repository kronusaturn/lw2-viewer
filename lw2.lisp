...
(defgeneric site-resources (site)
  ...
  (:method append ((s site))
    (labels ((gen-inner (theme os color-scheme-preference)
	       (list :stylesheet (generate-versioned-link (format nil "/css/style~@[-~A~].~A.css" theme os))
		     :media (if color-scheme-preference (format nil "(prefers-color-scheme: ~A)" color-scheme-preference))
		     :class "theme"))
	     (gen-theme (theme os)
	       (if theme
		   (list (gen-inner theme os nil))
		   (list (gen-inner nil os "light")
			 (gen-inner "dark" os "dark")))))
